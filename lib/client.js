...
var logger = new Logger({ name: 'Client' })
...
function buildRequestOptions(opts) {
  var debug = opts.__debug__ || false
  var baseURL = (opts.baseURL || DEFAULT_URL) + '/api/' + API_VERSION
  var userToken = opts.userToken || null
  var locale = opts.locale || DEFAULT_LOCALE
  var promoCode = opts.promoCode
...
var Client = function(options) {
  var self = this
  var opts = options || {}
  logger.setDebug(opts.__debug__)
  ...
  self.buildRequest = function(route) {
    return buildRequest(baseURL, route, self.request, self.store)
  }
}
